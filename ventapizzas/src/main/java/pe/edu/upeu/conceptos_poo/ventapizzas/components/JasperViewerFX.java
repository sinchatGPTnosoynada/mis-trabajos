 ...
 * Modified to use Text Buttons to avoid NullPointerException on missing icons.
 *
...
public class JasperViewerFX extends Dialog<Void>{
    ...
    private BorderPane createContentPane() {
        // CORRECCIÓN: Usamos botones de texto para evitar errores si faltan las imágenes
        ...
        // Estilos opcionales para que se vean mejor
        String buttonStyle = "-fx-font-weight: bold; -fx-min-width: 40px;";
        btnPrint.setStyle(buttonStyle);
        btnSave.setStyle(buttonStyle);
        btnBackPage.setStyle(buttonStyle);
        btnFirstPage.setStyle(buttonStyle);
        btnNextPage.setStyle(buttonStyle);
        btnLastPage.setStyle(buttonStyle);
